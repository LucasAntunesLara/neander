 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class SetBreakpointDialog extends JDialog implements ItemListener, ActionListener, Define {

	/**
	 * @throws java.awt.HeadlessException
	 */
	public SetBreakpointDialog(Simulator parSim, Processor parProc) throws HeadlessException {
		super();
		sSim = parSim;
		pProc = parProc;
		...
		if ( arg0.getSource() == (Object) okButton) {
			...
			if ( ! error) {				
				if(pProc.sBreakpoint == null) {
					pProc.sBreakpoint = new SetBreakpoint();
				}			
				
				if(( Breakpoint) pProc.sBreakpoint.searchPrimitive(sComp) != null) {
					pProc.sBreakpoint.remove(sComp);
				}
				
				pProc.pBreakpoint = new Breakpoint();
				pProc.pBreakpoint.setName(sComp);
				pProc.pBreakpoint.mp = new MicroOperation ( );
				pProc.pBreakpoint.mp.setComponentName(sComp);
				pProc.pBreakpoint.mp.setPortType ( x);
				pProc.pBreakpoint.mp.setContents( lValue);
				
				pProc.sBreakpoint.add(pProc.pBreakpoint);				
