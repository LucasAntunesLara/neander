 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class GenericRegisterBankFigure extends TDSimFigure implements LinkSimulatorVisualization, Define {
	/**
	 * 
	 */
	public GenericRegisterBankFigure() {
	...
	public GenericRegisterBankFigure(String parName, Processor parProc) {
		int i;
		SetPort spAux;
		...
		// +1 para tipo de escrita: full, left or right
		...
		try{
			...
			// no caso de nao existir a entrada de controle que define o tipo de escrita: FULL, HIGH, LOW
			sC [ i+iEntries] = "";
			sValueC [ i+iEntries] = "";
		}
	...
	protected void drawStrings (Graphics g) {
		...
		for ( i = 0; i < iOuts+1; i ++) {
			...
			int yDes = y + (int)(h*0.25);
			int hDes = (int) (h*0.5);
			if ( i < iOuts) g.drawString(sValueS[i],x+(int)(w*0.75),yDes+(int)(hDes*fDistanceNew*mult));
