 ...
 */
public class MultiplexerFigure extends TDSimFigure implements LinkSimulatorVisualization, Define {
	/**
	 * 
	 */
	public MultiplexerFigure() {
	...
	public MultiplexerFigure(String parName, String parE1, String parE2, 
	...
	private void drawPorts (Graphics g) {
		...
		x = r.x; y = r.y; w = r.width; h = r.height;
		g.setColor ( colorDraw);	
		g.drawLine(x,y+(int)(h*0.25),x+(int)(w*0.25),y+(int)(h*0.25));
		g.drawLine(x,y+(int)(h*0.75),x+(int)(w*0.25),y+(int)(h*0.75));
		g.drawLine(x+(int)(w*0.75),y+(int)(h*0.5),x+w,y+(int)(h*0.5));
		g.drawLine(x+(int)(w*0.5),y,x+(int)(w*0.5),y+(int)(h*0.166));
		g.setColor(getFrameColor());
	}
	...
	private void drawStrings (Graphics g) {
		...
		g.setFont(fb);
		g.drawString(sValueE1, x+(int)(w*0.10), y+(int)(h*0.25));
		g.drawString(sValueE2, x+(int)(w*0.10), y+(int)(h*0.75));
		g.drawString(sName, x+(int)(w*0.28), y+(int)(h*0.5));
		g.drawString(sValueS1,x+(int)(w*0.85),y+(int)(h*0.5));
		g.drawString(sValueSEL, x+(int)(w*0.60), y+(int)(h*0.20));
		g.setColor(getFrameColor());
	}
...
//		handles.add(new NullHandle(this, RelativeLocator.southEast()));
		handles.add(new ConnectionHandle(this, new RelativeLocator ( 0, 0.25),prototype));//RelativeLocator.southWest()));
//		handles.add(new NullHandle(this, RelativeLocator.northEast()));
		handles.add(new ConnectionHandle(this, new RelativeLocator ( 0, 0.75),prototype));
	...
	private void createConnectors() {
		...
		fConnectors.add(new PortConnector(this, RelativeLocator.east(),sS1) );
		fConnectors.add(new PortConnector(this, new RelativeLocator(0,0.25),sE1) );
		fConnectors.add(new PortConnector(this, new RelativeLocator(0,0.75),sE2) );
	}
...
//		setFont(fb);
	...
	public void refreshDisplay ( Simulator parSim, Processor parProc) {
		...
		if ( ( pAux = cCircuit.getPort( sE1, IN)) != null) {
			iTypeValue = pAux.getTypeValue();
			lTmp = dtp.execute ( sName, GET, sE1, IN);
			sValueE1 = SistNum.printInformation ( lTmp, iTypeValue);
		}
		if ( ( pAux = cCircuit.getPort( sE2, IN)) != null) {
			iTypeValue = pAux.getTypeValue();
			lTmp = dtp.execute ( sName, GET, sE2, IN);
			sValueE2 = SistNum.printInformation ( lTmp, iTypeValue);
		}
		if ( ( pAux = cCircuit.getPort( sS1, OUT)) != null) {
			iTypeValue = pAux.getTypeValue();
			lTmp = dtp.execute ( sName, GET, sS1, OUT);
			sValueS1 = SistNum.printInformation ( lTmp, iTypeValue);
		}
