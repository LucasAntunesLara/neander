 ...
 */
public class RegisterV2Figure extends TDSimFigure implements LinkSimulatorVisualization, Define {
	/**
	 * 
	 */
	public RegisterV2Figure() {
	...
	public RegisterV2Figure(String parName, String parE1, String parS1) {
		initialize();
		fConnectors = null;
		sName = parName;
		sE1 = parE1;
	...
	private void drawPorts (Graphics g) {
		...
		x = r.x; y = r.y; w = r.width; h = r.height;
		g.setColor ( colorDraw);
		g.drawLine(x+(w/2),y,x+(w/2),y+(int)(h*0.25));
		g.drawLine(x+(w/2),y+(int)(h*0.75),x+(w/2),y+w);
		g.setColor(getFrameColor());
	}
	...
	private void drawStrings (Graphics g) {
		...
		g.setFont(fb);
		g.drawString(sValueE1, x+(int)(w*0.25), y+h);
		g.drawString(sValueS1,x+(int)(w*0.55),y+(int)(h*0.23));
		fb = new Font("Helvetica", Font.PLAIN, 10);
		g.setFont(fb);
		g.drawString(sName+"="+sContent, x+(int)(w*0.03), y+(int)(h*0.5));	
		g.setColor(getFrameColor());
	}
...
//		handles.add(new NullHandle(this, RelativeLocator.northWest()));
	...
	private void createConnectors() {
		...
		pcE1 = new PortConnector(this, RelativeLocator.south(),sE1);
		pcS1 = new PortConnector(this, RelativeLocator.north(),sS1);
